(function () {
  ...
  // Supported here: raw indexed, RLE8 indexed, and embedded PNG8/24/32.
  // RLE5/LZ5 remain safe fallbacks until their codecs are added.
  // ---------------------------------------------------------------------
  ...
  function sff2Palette(bytes, dir, paletteIndex) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      out[d + 2] = bytes[s + 2];
      out[d + 3] = bytes[s + 3];
      if (bytes[s + 3]) anyAlpha = true;
    }
    // Some older v2 writers leave the fourth palette byte zeroed rather than
    // using it as alpha. Treat those as opaque except color index 0.
    if (!anyAlpha) {
      for (let i = 1; i < count; i++) out[i * 4 + 3] = 255;
    }
    out[3] = 0;
    return out;
  }
  ...
  function decodeSff2Rle8(bytes, start, length, pixelCount) {
    if (length < 5 || start < 0 || start + length > bytes.length) return null;
    let p = start + 4, out = 0;
    const indices = new Uint8Array(pixelCount);
    while (p < start + length && out < pixelCount) {
      const b = bytes[p++];
      if ((b & 0xC0) === 0x40) {
        const run = b & 0x3F;
        if (p >= start + length) return null;
        const value = bytes[p++];
  ...
  function decodeSff2Portrait(bytes, dir, cand) {
    ...
    const base = e.flags === 0 ? dir.ldataOffset : dir.tdataOffset;
    ...
    let indices = null;
    if (e.format === 0 && e.depth === 8) {
      const need = width * height;
      let p = start;
      if (e.dataLength >= need + 4) {
        const dv = new DataView(bytes.buffer, bytes.byteOffset + start, e.dataLength);
        if (dv.getUint32(0, true) === need) p += 4;
      }
      if (p + need <= start + e.dataLength) indices = bytes.slice(p, p + need);
    } else if (e.format === 2 && e.depth === 8) {
      indices = decodeSff2Rle8(bytes, start, e.dataLength, width * height);
    } else {
      return { url:null, reason:'SFF v2 format ' + e.format + '/depth ' + e.depth + ' pending', format:e.format };
    }

    if (!indices) return { url:null, reason:'indexed pixel decode failed', format:e.format };
    const palette = sff2Palette(bytes, dir, requested.paletteIndex);
    ...
    return url ? { url, width, height, format:e.format, kind:e.format === 2 ? 'rle8' : 'raw' } : { url:null, reason:'canvas encode failed' };
  }
  ...
  const PORTRAIT_OVERRIDES = Object.freeze({
    ...
    kineza: '../kineza_full.png?v=579cf39e-v22'
  });
  ...
  async function refreshSelectedPortraits() {
    ...
    const diagParts = ['BUILD: V22', 'SELFTEST: ' + portraitSelfTestResult];
    ...
    while (rosterThumbActive < ROSTER_THUMB_CONCURRENCY && rosterThumbQueue.length) {
      ...
      Promise.resolve(loadPortraitForCharacter(name)).then(result => {
        ...
        if (result && result.url && img) {
          ...
          btn.classList.add('thumb-fallback');
          btn.dataset.thumbState = 'fallback';
          log('I29 THUMB · fallback ' + name + ' · ' + ((result && result.status) || 'no portrait'));
        }
      }).catch(err => {
