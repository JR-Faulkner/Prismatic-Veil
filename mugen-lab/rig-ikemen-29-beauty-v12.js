(function () {
  ...
  async function loadPortrait9000x1(name) {
    name = String(name || '');
    if (!name) return null;
    ...
    const promise = (async () => {
      try {
        ...
        if (parsed.version !== 1) throw new Error('SFF v' + parsed.version + ' portrait decode not enabled yet');

        const idx = parsed.entries.findIndex(e => e.group === 9000 && e.image === 1);
        if (idx < 0) throw new Error('9000,1 not found');
        const linked = resolveSff1Linked(parsed.entries, idx);
        if (!linked) throw new Error('9000,1 link could not be resolved');
        ...
        if (!decoded) throw new Error('9000,1 PCX/palette could not be decoded');
        const url = rgbaToDataUrl(decoded);
        if (!url) throw new Error('canvas conversion failed');

        log('I29 PORTRAIT · ' + name + ' 9000,1 decoded from ' + sffPath + ' (' + decoded.width + 'x' + decoded.height + ')');
        return url;
      } catch (e) {
        log('I29 PORTRAIT · ' + name + ' fallback -- ' + ((e && e.message) || e));
        return null;
      }
    })();
  ...
  async function refreshSelectedPortraits() {
    ...
    const pairs = [
      [document.getElementById('selP1'), document.getElementById('p1Portrait')],
      [document.getElementById('selP2'), document.getElementById('p2Portrait')],
    ];
    await Promise.all(pairs.map(async ([label, well]) => {
      if (!label || !well) return;
      const name = (label.textContent || '').trim();
      if (!name || name === '-' || /choose/i.test(name)) {
        well.classList.remove('has-art');
        well.style.backgroundImage = '';
        return;
      }
      const url = await loadPortrait9000x1(name);
      if (token !== portraitRefreshToken) return;
      const current = (label.textContent || '').trim();
      if (current !== name) return;
      if (url) {
        well.style.backgroundImage = 'url("' + url + '")';
