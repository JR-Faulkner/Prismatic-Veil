(function(){
  'use strict';
  const base = './rig-f10-8.js?v=f1018-base-f10-8';
  fetch(base, { cache: 'no-store' })
    .then(function(r){ if(!r.ok) throw new Error('F10.18 base JS HTTP '+r.status); return r.text(); })
    .then(function(src){
            {
        const copyAnchor="const text='MOBMUGEN \u00b7 RIG F \u00b7 F10.8 JIT FAULT WITNESS\\nRUNTIME STATUS \u00b7 '+state.textContent+'\\nPAGE \u00b7 '+location.href+'\\nUSER AGENT \u00b7 '+navigator.userAgent+'\\n\\nTRACE\\n'+lines.join('\\n')+'\\nF10.8 PERF SNAPSHOT \u00b7 '+(window.F105_PERF||'pending')+'\\nF10.8 FAULTS \u00b7 '+faultSummary()+'\\nF10.8 HEAP \u00b7 bytes='+(window.F108_HEAP||'?')+' max='+(window.F108_HEAPMAX||'?')+' grows='+(window.F108_GROWS||0)+' refused='+(window.F108_GROW_FAIL||0);";
        if(!src.includes(copyAnchor))throw new Error('F10.18 copy-text anchor not found in F10.8 base -- base script changed, patch is stale');
        src=src.replace(copyAnchor,"const text='MOBMUGEN \u00b7 RIG F \u00b7 F10.8 JIT FAULT WITNESS\\nRUNTIME STATUS \u00b7 '+state.textContent+'\\nPAGE \u00b7 '+location.href+'\\nUSER AGENT \u00b7 '+navigator.userAgent+'\\n\\nPINNED CRASH DETAIL (captured once, survives trace rotation)\\n'+(pinned.length?pinned.join('\\n'):'none captured yet')+'\\n\\nTRACE (most recent, may have rotated past earlier events)\\n'+lines.join('\\n')+'\\nF10.8 PERF SNAPSHOT \u00b7 '+(window.F105_PERF||'pending')+'\\nF10.8 FAULTS \u00b7 '+faultSummary()+'\\nF10.8 HEAP \u00b7 bytes='+(window.F108_HEAP||'?')+' max='+(window.F108_HEAPMAX||'?')+' grows='+(window.F108_GROWS||0)+' refused='+(window.F108_GROW_FAIL||0);");
      }
src = src.replaceAll('F10.8 JIT FAULT WITNESS', 'F10.18 PINNED CRASH DETAIL');

      src = src.replaceAll('F10.8', 'F10.18');
      src = src.replaceAll('F108_', 'F1018_');
      src = src.replaceAll('mobmugen-f108-writtenjitoff-trace', 'mobmugen-f1018-pinned-trace');
      ...
      // while these repeated dozens of times in the visible trace.
      {
        ...
        if(!src.includes(summaryAnchor))throw new Error('F10.18 faultSummary anchor not found in F10.8 base -- base script changed, patch is stale');
        ...
        if(!src.includes(siftAnchor))throw new Error('F10.18 sift-head anchor not found in F10.8 base -- base script changed, patch is stale');
        src=src.replace(siftAnchor,"function sift(tag,s){\n  const rd=s.match(/^([0-9A-Fa-f]{8})\\s+EAX=[0-9A-Fa-f]{8}\\s+ECX=[0-9A-Fa-f]{8}\\s+EDX=[0-9A-Fa-f]{8}\\s+EBX=[0-9A-Fa-f]{8}\\s+ESP=[0-9A-Fa-f]{8}\\s+EBP=[0-9A-Fa-f]{8}\\s+ESI=[0-9A-Fa-f]{8}\\s+EDI=[0-9A-Fa-f]{8}\\s+(.+?)\\s+at\\s+([0-9A-Fa-f]{8})/);\n  if(rd){const mod=rd[2].trim();crashMods.set(mod,(crashMods.get(mod)||0)+1);crashHits++;\n    if(crashShown<2){crashShown++;const line='F10.18 CRASH SITE \u00b7 '+s.slice(0,1200);log(line);pinned.push(line)}\n    else if(crashHits-crashLastSummary>=50){crashLastSummary=crashHits;log('F10.18 CRASH SITE SUMMARY \u00b7 '+crashHits+' \u00b7 '+[...crashMods.entries()].sort((a,b)=>b[1]-a[1]).slice(0,6).map(([a,n])=>a+' x'+n).join(', '))}\n    return}\n  const sw=s.match(/^\\s*(\\S[\\S ]*?)\\s{2,}\\S*\\s*([0-9A-Fa-f]{8})\\s*\\/\\s*([0-9A-Fa-f]{8})\\s*$/);\n  if(sw){const mod=sw[1].trim();stackMods.set(mod,(stackMods.get(mod)||0)+1);stackHits++;\n    if(stackShown<10){stackShown++;const line='F10.18 STACK \u00b7 '+s.slice(0,1200);log(line);pinned.push(line)}\n    else if(stackHits-stackLastSummary>=100){stackLastSummary=stackHits;log('F10.18 STACK SUMMARY \u00b7 '+stackHits+' \u00b7 '+[...stackMods.entries()].sort((a,b)=>b[1]-a[1]).slice(0,6).map(([a,n])=>a+' x'+n).join(', '))}\n    return}\n  const uhf=s.match(/Unhandled page fault on read access to\\s*(0x[0-9A-Fa-f]+)/i);\n  if(uhf){const addr=uhf[1].toUpperCase();unhFaults++;unhAddrs.set(addr,(unhAddrs.get(addr)||0)+1);\n    if(unhShown<2){unhShown++;log('BW '+tag+' \u00b7 '+s.slice(0,1200));return}\n    if(unhFaults-unhLastSummary>=50){unhLastSummary=unhFaults;log('F10.18 UNHANDLED-FAULT SUMMARY \u00b7 '+unhFaults+' \u00b7 '+[...unhAddrs.entries()].sort((a,b)=>b[1]-a[1]).slice(0,4).map(([a,n])=>a+' x'+n).join(', '))}\n    return}\n  const uic=s.match(/to unimplemented function\\s*([^,]+),\\s*abort/i);\n  if(uic){const name=uic[1].trim();unimplCalls++;unimplNames.set(name,(unimplNames.get(name)||0)+1);\n    if(unimplShown<2){unimplShown++;log('BW '+tag+' \u00b7 '+s.slice(0,1200));return}\n    if(unimplCalls-unimplLastSummary>=50){unimplLastSummary=unimplCalls;log('F10.18 UNIMPLEMENTED-CALL SUMMARY \u00b7 '+unimplCalls+' \u00b7 '+[...unimplNames.entries()].sort((a,b)=>b[1]-a[1]).slice(0,4).map(([a,n])=>a+' x'+n).join(', '))}\n    return}\n  const fm=s.match(/Page Fault at\\s*([0-9A-Fa-f]+)/);\n  if(fm){");
      }
      ...
      // contrary to its own stated purpose. Fixed by patching `ss` itself.
      {
        ...
        if(!src.includes(shellFetchAnchor))throw new Error('F10.18 shell-fetch anchor not found in F10.8 base -- base script changed, patch is stale');
        src=src.replace(shellFetchAnchor,"let ss=await sr.text();ss+='\\n;window.__F105_Config=Config;window.__F105_setConfiguration=setConfiguration;window.__F105_start=start;window.__F105_startEmulator=startEmulator;window.__F105_Module=Module;';"+
"const F1018_explorerNeedle=/else\\s*\\{\\s*params\\.push\\(\"explorer\"\\);\\s*params\\.push\\(\"\\/desktop=shell\"\\);\\s*\\}/;if(!F1018_explorerNeedle.test(ss))throw new Error('F10.18 explorer-fallback needle not found in fetched shell -- upstream shell text changed, patch is stale');ss=ss.replace(F1018_explorerNeedle,'else{console.log(\"F10.18 HARNESS \\u00b7 cmd.exe launch \\u00b7 no explorer fallback\");}');");
      }
      src = src.replace('written-code JIT disabled · Wine 1.7.55 proven root', 'pinned crash detail · cmd.exe PE-execution probe · Wine 1.7.55 proven root');
      src = src.replace('const BASE=', "console.log('F10.18 WRAPPER · loaded F10.8 base · same F10.15 test, pinned crash detail survives trace rotation');\nconst BASE=");
      ...
      s.text = src + "\n//# sourceURL=rig-f10-18.generated.js";
      document.head.appendChild(s);
    })
    .catch(function(e){
      const msg = 'F10.18 WRAPPER ERROR · ' + ((e && e.stack) || e);
      console.error(msg);
      ...
      if (state) state.textContent = 'FAILED · F10.18 WRAPPER';
    });
})();
