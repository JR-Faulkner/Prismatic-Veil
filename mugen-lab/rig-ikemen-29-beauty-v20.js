(function () {
  ...
  const PORTRAIT_OVERRIDES = Object.freeze({
    ...
    kineza: '/Prismatic-Veil/kineza_full.png?v=579cf39e-v19'
  });
  ...
  async function refreshSelectedPortraits() {
    ...
    const diagParts = ['BUILD: V20', 'SELFTEST: ' + portraitSelfTestResult];
  ...
  function ensureRosterThumbObserver() {
    if (rosterThumbObserver || typeof IntersectionObserver === 'undefined') return rosterThumbObserver;
    rosterThumbObserver = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        if (!entry.isIntersecting) return;
        rosterThumbObserver.unobserve(entry.target);
        enqueueRosterThumbnail(entry.target);
      });
    }, { root:null, rootMargin:'140px 0px', threshold:0.01 });
    return rosterThumbObserver;
  }
  ...
  function wireRosterThumbnail(btn, name) {
    ...
    const observer = ensureRosterThumbObserver();
    if (observer) observer.observe(btn);
    else enqueueRosterThumbnail(btn);
  }
