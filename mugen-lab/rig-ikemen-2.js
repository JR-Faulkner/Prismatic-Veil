(function () {
  ...
  function pin(tag, s) {
    if (pinned.length < 60) pinned.push('I2 ' + tag + ' · ' + String(s).slice(0, 1400));
  }
  ...
  globalThis.fs = {
    ...
    stat(path, callback) {
      const orig = path;
      path = resolvePath(normPath(path));
      if (window.__I2_TRACE_STAT) console.log('I2 STAT TRACE ·', orig, '->', path, '->', vfsExists(path));
  ...
  log('I2 VFS ARMED · in-memory filesystem installed before wasm_exec.js loads');
  ...
  async function loadRuntimeAssets() {
    ...
    log('I2 RUNTIME ASSETS · fetching ikemen-runtime-assets.zip (engine\'s own data/font/external -- required regardless of game content, per RIG I2\'s first probe hitting "open external/script/main.lua")');
    ...
    log('I2 RUNTIME ASSETS LOADED · ' + n + ' engine files into VFS (data/, font/, external/)');
  ...
  zipInput.addEventListener('change', async () => {
    ...
    try {
      ...
      const msg = 'I2 ZIP ERROR · ' + ((e && e.stack) || e);
  ...
  async function boot() {
    ...
    try {
      ...
      log('I2 WRAPPER · loaded rig-ikemen-2 · VFS + CLI quick-match boot probe');
      log('I2 USER AGENT · ' + navigator.userAgent);
      ...
      argv.push('-p2.ai', '5');
      go.argv = argv;
      log('I2 ARGV · ' + JSON.stringify(argv));
      ...
      const msg = 'I2 BOOT ERROR · ' + ((e && e.stack) || e);
      log(msg); pin('CRASH', msg); status('FAILED · SEE TRACE');
    }
  }
  ...
  copyBtn.addEventListener('click', async () => {
    ...
    const text = 'MOBMUGEN · IKEMEN · RIG I2 VFS PROBE\n' +
  ...
  log('I2 READY · tap CHOOSE ZIP, then START PROBE');
})();
