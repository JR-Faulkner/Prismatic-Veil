...
pcu:{name:'PCU / B2',icon:'🎮',category:'tech',condition:92,integrity:88,efficiency:68,comfort:0,performance:76,worth:24,mods:[],claimType:'owned'},
...
const ICON_ROOT='./assets/ui/behemoth2/icons/v42134/';
const ITEM_GLYPHS={car:'virog_4',pcu:'pcu_b2',tools:'starter_tool_kit',beacon:'survey_beacon',fieldpack:'field_pack',relay:'base_property'};
const CAT_GLYPHS={vehicle:'virog_4',tech:'pcu_b2',equipment:'starter_tool_kit',base:'base_property',other:'claim'};
function glyphImg(key,extra=''){return '<img class="itemGlyph '+extra+'" src="'+ICON_ROOT+key+'.png?v=v4.21.34" alt="" aria-hidden="true">'}
function itemGlyph(id,extra=''){return glyphImg(ITEM_GLYPHS[id]||'claim',extra)}
function categoryGlyph(cat){return glyphImg(CAT_GLYPHS[cat]||'claim','categoryGlyph')}
function claimGlyph(){return glyphImg('claim','claimGlyph')}
...
const INTERNALS={
 ...
 pcu:{
  basic:'Inside the PCU / B2 you can distinguish the main board, compute package, memory, storage, cooling assembly, power delivery, wireless hardware, ports and the physical shell.',
...
function resolveTargetFromText(low){
 ...
 if(/\bpcu\b|\bb2\b|behemoth\s*2|game|console|gaming system/.test(low))return'pcu';
...
function updateActionScroll(){
 const box=$('result'),wrap=box&&box.closest('.actionLogShell'),thumb=wrap&&wrap.querySelector('.actionScrollThumb'),rail=wrap&&wrap.querySelector('.actionScroll');
 if(!box||!thumb||!rail)return;
 const max=Math.max(0,box.scrollHeight-box.clientHeight);
 rail.classList.toggle('noScroll',max<=1);
 const track=rail.clientHeight||1;
 const thumbH=max<=1?track:Math.max(18,track*(box.clientHeight/box.scrollHeight));
...
function render(){
 ...
 const sc=scene();updateSceneWindow();$('thread').textContent=sc.thread;$('mood').textContent=sc.mood;$('sceneTitle').textContent=sc.title;$('sceneText').textContent=sc.text;const ns=$('narrativeScroll');if(ns)ns.scrollTop=0;$('result').textContent=S.result||'';$('result').scrollTop=0;requestAnimationFrame(updateActionScroll);
 $('eventChip').className='eventChip'+(S.currentEvent?' show':'');$('eventChip').textContent=S.currentEvent?'✦ '+sc.chip:'';
 $('choices').innerHTML='';sc.choices.slice(0,5).forEach((c,i)=>{const key=choiceKey(c[2]),used=!!(S.usedChoices&&S.usedChoices[key]),b=document.createElement('button');b.className='choice'+(used?' used':'');b.type='button';b.disabled=used;b.dataset.choiceState=used?'used':'default';b.innerHTML='<img class="choiceArt choiceArtDefault" src="./assets/ui/behemoth2/production/choice_row_default.svg?v=v4.21.31" alt=""><img class="choiceArt choiceArtActive" src="./assets/ui/behemoth2/production/choice_row_active.svg?v=v4.21.31" alt=""><img class="choiceArt choiceArtUsed" src="./assets/ui/behemoth2/production/choice_row_used.svg?v=v4.21.31" alt=""><img class="choiceArt choiceArtLocked" src="./assets/ui/behemoth2/production/choice_row_locked.svg?v=v4.21.31" alt=""><span class="choiceIndex">'+(used?'<span class="usedMark">✓</span>':digitalHTML(String(i+1).padStart(2,'0')))+'</span><span class="choiceIcon">'+choiceIcon(c[2],c[0])+'</span><span class="choiceBody"><strong>'+c[0]+'</strong><small>'+c[1]+'</small></span><span class="choiceState">'+(used?'USED':((String(c[3]).includes('%'))?digitalHTML(c[3]):escapeHtml(c[3])))+'</span>';b.onclick=()=>{S.usedChoices=S.usedChoices||{};S.usedChoices[key]=true;handle(c[2])};$('choices').appendChild(b)});
...
function renderCategories(){
 ...
 if(st===0){
  ...
  Object.entries(S.items).forEach(([id,x])=>{
   ...
   b.innerHTML='<span class="icon">'+itemGlyph(id)+'</span><span><strong>'+x.name+'</strong><small>'+((x.claimType==='recovered')?'Recovered Claim':'Established Claim')+'</small></span><span class="claimMark">'+claimGlyph()+'<b>CLAIM</b></span>';
 ...
 [...new Set(Object.values(S.items).map(x=>x.category))].forEach(cat=>{
  ...
  d.innerHTML='<summary><span class="catIcon">'+categoryGlyph(cat)+'</span><span><strong>'+meta.name+'</strong><span class="catMeta">'+entries.length+' '+(entries.length===1?'claim':'claims')+(st>=2?' · '+meta.focus.toUpperCase()+' '+stat:'')+'</span></span><span class="catValue">'+(S.worthUnlocked&&st>=2?'W '+worth:'DETAILS')+'</span></summary><div class="catItems"></div>';
  const inner=d.querySelector('.catItems');entries.forEach(([id,x])=>{const b=document.createElement('button');b.className='prop'+(id===S.selected?' active':'');b.type='button';b.innerHTML='<span class="icon">'+itemGlyph(id)+'</span><span><strong>'+x.name+'</strong><small>'+(x.mods.length?x.mods.slice(-2).join(' · '):(x.claimType==='recovered'?'Recovered claim':'Established claim'))+'</small></span><span class="mini">'+(S.worthUnlocked&&st>=2?'W '+x.worth:'COND '+x.condition)+'</span>';b.onclick=()=>{S.selected=id;render()};inner.appendChild(b)});box.appendChild(d)
 });
}
function renderSelected(){
 const x=S.items[S.selected]||Object.values(S.items)[0];if(!x)return;const st=alterStage();updateAlterSense();
 $('propName').textContent=x.name;$('propMods').textContent=st===0?'CLAIMED':(x.mods.length?(x.mods.length+' MOD'+(x.mods.length===1?'':'S')):'CLAIMED');const art=$('claimArt');if(art){art.className='claimArt'+(S.selected==='car'?' vehicle':'');art.innerHTML=itemGlyph(S.selected,'selectedClaimGlyph')}
...
function freeAction(text){
 ...
 const low=t.toLowerCase().replace(/[’‘]/g,"'"),target=resolveTargetFromText(low);
 const wantsContents=/what(?:'s| is)\s+(?:inside|in)|what tools do i have|show\s+(?:the\s+)?contents|list\s+(?:the\s+)?tools|what came in (?:the\s+)?kit|open\s+(?:the\s+)?(?:toolkit|tool kit)|contents?\s+of/.test(low);
 if(wantsContents){const contentTarget=/tool|gear|kit/.test(low)?'tools':target;markTypedEquivalent('deep',contentTarget);deepInspect(contentTarget);render();return}
...
if(S){S.premise=BACKGROUND;if(S.items&&S.items.console&&!S.items.pcu){S.items.pcu=S.items.console;delete S.items.console}if(S.items&&S.items.pcu){S.items.pcu.name='PCU / B2';S.items.pcu.category='tech';}if(S.selected==='console')S.selected='pcu';S.claims=(S.claims||Object.keys(S.items||{})).map(id=>id==='console'?'pcu':id);S.claims=[...new Set(S.claims)];if(S.items&&S.items.car){S.items.car.name='Virog 4';S.items.car.icon='🚙';}S.knowledge=S.knowledge||{'alteration sense':1,'claim sense':1};S.journal=S.journal||[];S.claims=S.claims||Object.keys(S.items||{});S.threads=S.threads||{business:true,serviceSpur:false,oldTraveler:false,uncle:false};S.usedChoices=S.usedChoices||{};S.baseAP=S.baseAP||6;S.result=S.result||'Welcome back to the Verdant.';render()}
})();
