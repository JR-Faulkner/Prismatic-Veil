...
function render(){
 ...
 const sc=scene();updateSceneWindow();$('thread').textContent=sc.thread;$('mood').textContent=sc.mood;$('sceneTitle').textContent=sc.title;$('sceneText').textContent=sc.text;$('result').textContent=S.result||'';$('result').scrollTop=0;requestAnimationFrame(updateActionScroll);
 $('eventChip').className='eventChip'+(S.currentEvent?' show':'');$('eventChip').textContent=S.currentEvent?'✦ '+sc.chip:'';
 $('choices').innerHTML='';sc.choices.slice(0,5).forEach((c,i)=>{const key=choiceKey(c[2]),used=!!(S.usedChoices&&S.usedChoices[key]),b=document.createElement('button');b.className='choice'+(used?' used':'');b.type='button';b.disabled=used;b.dataset.choiceState=used?'used':'default';b.innerHTML='<img class="choiceArt choiceArtDefault" src="./assets/ui/behemoth2/production/choice_row_default.svg?v=v4.21.10" alt=""><img class="choiceArt choiceArtActive" src="./assets/ui/behemoth2/production/choice_row_active.svg?v=v4.21.10" alt=""><img class="choiceArt choiceArtUsed" src="./assets/ui/behemoth2/production/choice_row_used.svg?v=v4.21.10" alt=""><img class="choiceArt choiceArtLocked" src="./assets/ui/behemoth2/production/choice_row_locked.svg?v=v4.21.10" alt=""><span class="choiceIndex">'+(used?'<span class="usedMark">✓</span>':digitalHTML(String(i+1).padStart(2,'0')))+'</span><span class="choiceIcon">'+choiceIcon(c[2],c[0])+'</span><span class="choiceBody"><strong>'+c[0]+'</strong><small>'+c[1]+'</small></span><span class="choiceState">'+(used?'USED':((String(c[3]).includes('%'))?digitalHTML(c[3]):escapeHtml(c[3])))+'</span>';b.onclick=()=>{S.usedChoices=S.usedChoices||{};S.usedChoices[key]=true;handle(c[2])};$('choices').appendChild(b)});
