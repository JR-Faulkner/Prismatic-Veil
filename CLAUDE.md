...
**A texture rendered far smaller than its source resolution reads as translucent, even at `alpha: 1`.** `TacticalScene`'s hero map tokens first loaded Prismel's and Auryi's full battle-res art (533×800 / 1024×1536) directly and let Phaser scale it down live to a ~40px on-screen token — a ~20x GPU minification. User-reported symptom: "the sprites are literal ghosts." Nothing was actually transparent in code (`sprite.alpha`, `container.alpha`, tint all confirmed `1`/none) — the haze came from linear-filtered downscaling averaging huge numbers of soft, semi-transparent edge pixels (hair, cloak fringe, ambient glow) into each screen pixel; simulating the same downscale offline confirmed mean alpha in the resampled result dropped from ~255 to ~199. First fix pass (pre-resize to a `map_icons/` copy targeting ~180px content height, plus a backing disc) wasn't enough — still reported ghostly. Root cause held, but the fix was undersized: bumped the `map_icons` reference size to 260px content height (was 180) and the runtime on-screen target from 60 to 78 world px, verified this time by reading actual rendered canvas pixels back (not just eyeballing a screenshot) — at native resolution the character art is fully solid and saturated, no haze. **A UI element's `alpha` property being correct doesn't mean it will render opaque — if a texture is being scaled down by more than roughly 5–10x, resize the source asset itself rather than trusting the renderer's live minification, especially for art with soft/anti-aliased edges — and verify a fix like this by sampling actual rendered pixels, not by eyeballing a screenshot that's been shrunk for viewing, which hides exactly the kind of softness being fixed.**
