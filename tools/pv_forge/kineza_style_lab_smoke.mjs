...
const url = process.env.PRIZIM_STYLE_URL || 'http://127.0.0.1:4173/kineza-attack-style-lab.html?prizim=KINEZA-STYLE-AUDITION-003';
const expectedMarker = process.env.PRIZIM_BUILD_MARKER || 'KINEZA-STYLE-AUDITION-003';
...
try {
  ...
  await page.waitForFunction(() => window.__KINEZA_STYLE_LAB__?.ready === true, null, { timeout: 20000 });
  ...
  for (let style = 0; style < 3; style++) {
    ...
    for (let frame = 0; frame < 6; frame++) {
      ...
      if (state.styleIndex !== style || state.frameIndex !== frame) throw new Error(`Selection mismatch: expected style ${style} frame ${frame}, got style ${state.styleIndex} frame ${state.frameIndex}`);
      if (!state.canvasHasPixels) throw new Error(`Rendered canvas blank at style ${style} frame ${frame}`);
    }
  }

  await page.locator('#compare').click();
  await page.waitForFunction(() => window.__KINEZA_STYLE_LAB__?.getState().playing === false, null, { timeout: 15000 });
  ...
  for (const frames of visited) {
    if (frames.length !== 6 || frames.some((value, index) => value !== index)) throw new Error(`Incomplete frame coverage: ${JSON.stringify(frames)}`);
  }

  const geometry = await page.evaluate(() => {
    const selectors = ['#styles', '.stage', '.bottom', '#play', '#compare'];
    const vw = document.documentElement.clientWidth, vh = document.documentElement.clientHeight;
    return { viewport:[vw,vh], overflowX:document.documentElement.scrollWidth > vw + 2,
      elements:Object.fromEntries(selectors.map(selector => { const el=document.querySelector(selector); if(!el)return [selector,null]; const r=el.getBoundingClientRect(); return [selector,{left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height}]; })) };
  });
  ...
  for (const [selector, rect] of Object.entries(geometry.elements)) {
    ...
    if (rect.left < -2 || rect.right > geometry.viewport[0] + 2 || rect.top < -2 || rect.bottom > geometry.viewport[1] + 2) throw new Error(`Offscreen UI element ${selector}: ${JSON.stringify(rect)} viewport=${JSON.stringify(geometry.viewport)}`);
  }
  if (errors.length) throw new Error(errors.join('\n'));
  console.log(JSON.stringify({tool:'PriZim Kineza Style Lab Delivery Gate',status:'success',build_marker:expectedMarker,url,viewport:`${width}x${height}`,styles:3,frames_per_style:6,frame_coverage:'18/18',compare_cycle:'success',canvas_render:'success',ui_bounds:'success'}, null, 2));
