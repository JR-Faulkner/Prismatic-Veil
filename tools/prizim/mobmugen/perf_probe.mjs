...
async function sampleWindow(ms) {
  return await page.evaluate(async (duration) => {
    ...
    return {
      ...
      draw_ops_per_sec: drawDelta / seconds,
...
const samples=[];
for (let i=0; i<samplesWanted; i++) samples.push(await sampleWindow(sampleMs));
...
const drawOps = median(samples.map(x=>x.draw_ops_per_sec));
...
const usable = loopFps > 0.5 && rafFps > 10 && fatalHits.length === 0;
const schedulerHeadroom = rafFps > 0 ? Math.max(0, Math.min(1, loopFps / rafFps)) : 0;

const report={
  suite:'PriZim MOBMUGEN DirectDraw performance isolation',
  brand,
  route,
  fixture,
  workload,
  ...
  median_draw_ops_per_sec:drawOps,
  median_raf_fps:rafFps,
  loop_to_raf_ratio:schedulerHeadroom,
  target_45_met:loopFps >= 45,
  target_60_met:loopFps >= 58,
  ...
  note:'median_loop_fps is the authoritative saturation metric for A/B work. RAF is only browser scheduling cadence. draw_ops_per_sec counts internal BoxedWine drawing activity and is intentionally NOT labeled FPS.'
};
