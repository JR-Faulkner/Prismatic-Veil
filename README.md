...
Sprite tokens load pre-downsampled `map_icons/` copies of the character art (`assets/prismel/walk/map_icons/`, `assets/auryi/movement/map_icons/`), not the full battle-res source directly. Loading a 533×800/1024×1536 texture and letting the GPU minify it live down to a ~40px token is a ~20x downscale — linear-filtered minification at that ratio averages so many soft, semi-transparent edge pixels (hair, cloak fringe, ambient glow) into each screen pixel that the effective alpha drops to roughly 78%, reading as a translucent ghost rather than a small solid sprite. The `map_icons` are pre-resized once (LANCZOS, offline) to keep the runtime downscale in the 2–5x range a GPU linear filter handles cleanly, and every sprite token also sits on a solid accent-colored backing disc (same gold-ring language as the procedural tokens) so full-detail illustrated art has a grounded, contrasted base against the tile art instead of floating on nothing.
