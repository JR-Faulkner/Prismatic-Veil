...
### Movement masters — v0.5E Tactical Movement Masters
Replaced Auryi's and Prismel's tactical walk-cycle art with cleaned illustrated masters, and — new territory — gave both enemies illustrated movement tokens for the first time; they previously only ever rendered as the plain procedural circle from `_buildUnitToken()`. `ENEMY_TOKEN_ART` mirrors `CHARACTER_TOKEN_ART` exactly (same six-frame idle/walk shape, same `originY`/`scale`/`baseFacing` fields), and `_buildEnemy()` now branches on it the same way `_buildHero()` already branched on `CHARACTER_TOKEN_ART` — art if present, the old procedural token as fallback otherwise. `_buildCharacterToken()` itself just took an `artTable` parameter so both build paths share the one implementation. Enemies actually path and animate-move on their own turn already (`enemyAttack()`'s move-toward-target step calls the same `UnitController.animateMove()` heroes use), so the new walk cycles are exercised for real during a normal enemy phase, not just standing idle.

This handoff's own party-scale lock (`Auryi tallest > Prismel middle > Kineza shortest`, explicit reference art included) directly named the mechanism behind a scale mistake made earlier in the same integration pass — see `CLAUDE.md`'s traps list for the full story. Short version: Kineza's own dedicated v0.5B handoff had already locked a flat `0.294` runtime scale for him specifically, with relative party size meant to live entirely in each character's own pre-processed map-icon *content height* (his: 230px) rather than a second runtime multiplier. A prior pass mistook the shared `0.294` baseline as an oversight and imported Battle Presentation's `scaleMul` (a *different* number that also corrects for BP-canvas padding Kineza's tactical art never had) as that missing multiplier, shrinking his tactical token twice over two reports. v0.5E's asset processing restored the one-flat-scale-per-everyone system Kineza's own manifest had already established: Prismel unchanged (261px content height), Auryi resized to 344px (preserving her already-live, already-approved on-screen size rather than reverting to this package's own older 270px historical anchor), Kineza reverted to his locked 230px. Wraith (360px) and Hushling (400px, plus his own wider stance) had no prior anchor at all — enemies never had illustrated tactical art before — so those two are FAI's own calibrated choice against the handoff's qualitative direction ("purple spectral hover, imposing" / "massive... may exceed hero silhouettes, do not shrink to match").

A `sizeMul` field (content height relative to Prismel's 261px baseline) was added to every `CHARACTER_TOKEN_ART`/`ENEMY_TOKEN_ART` entry specifically to feed the per-character contact-shadow sizing in `_buildCharacterToken()` — with runtime `scale` now flat `0.294` for everyone, `art.idle.scale / 0.294` (the ratio the shadow used to read) always evaluates to `1`, silently losing the "a visibly bigger character doesn't stand on the same tiny shadow" fix from the same session; `sizeMul` is the value that actually varies per character now.
