...
function fadeOutMenuMusic(scene, duration = 550) {
  if (!gameState.titleMusic || !gameState.titleMusic.isPlaying) return;
  scene.tweens.add({
...
class SoundFX {
  ...
  noise(duration=0.08, volume=0.08, delay=0, filterFreq=900, filterType='bandpass') {
    if(!this.enabled || (gameState && gameState.sfxEnabled === false)) return;
    this.init();
    const t = this.ctx.currentTime + delay;
  ...
  // Prismel: glass shard + prism flicker
  attackPrismel() {
    this.tone(620, 0.055, 'triangle', 0.075, 920);
    this.tone(1240, 0.045, 'sine', 0.045, 1660, 0.025);
    this.noise(0.045, 0.035, 0.018, 2600, 'highpass');
  }

  // Kineza: kinetic punch, armor thump, green crackle
  attackKineza() {
    this.tone(96, 0.075, 'square', 0.17, 52);
    this.tone(240, 0.045, 'sawtooth', 0.08, 130, 0.012);
    this.noise(0.06, 0.075, 0.008, 850, 'bandpass');
    this.tone(720, 0.035, 'square', 0.035, 420, 0.02);
  }

  // Auryi: veil bloom + golden orb chime
  attackAuryi() {
    this.tone(196, 0.22, 'sine', 0.055, 260);
    this.tone(520, 0.16, 'triangle', 0.06, 780, 0.035);
    this.tone(1040, 0.12, 'sine', 0.045, 1320, 0.075);
    this.noise(0.09, 0.025, 0.035, 1800, 'bandpass');
  }

  // Enemy hit: crunchy impact tick
  hit() {
    this.tone(170, 0.045, 'square', 0.07, 82);
    this.noise(0.035, 0.045, 0.005, 700, 'bandpass');
  }

  // Enemy death: bit-crumble
  enemyDeath() {
    this.tone(260, 0.07, 'sawtooth', 0.09, 120);
    this.noise(0.10, 0.07, 0.02, 520, 'lowpass');
    this.tone(90, 0.08, 'square', 0.05, 48, 0.055);
  }

  // Resonance gem pickup
  gem() {
    this.tone(920, 0.04, 'triangle', 0.045, 1380);
    this.tone(1840, 0.035, 'sine', 0.022, 2100, 0.025);
  }

  // Player takes damage: low grit + flash
  playerHit() {
    this.tone(120, 0.12, 'sawtooth', 0.13, 54);
    this.noise(0.08, 0.08, 0.01, 460, 'bandpass');
  }

  // Level up: crystal fanfare, not toy-beep
  levelUp() {
    this.tone(392, 0.11, 'triangle', 0.08, 523);
    this.tone(523, 0.12, 'triangle', 0.085, 784, 0.105);
    this.tone(784, 0.16, 'sine', 0.09, 1175, 0.215);
    this.noise(0.18, 0.045, 0.12, 3000, 'highpass');
  }

  // Menu confirm: arcade crystal snap
  select() {
    this.tone(420, 0.045, 'square', 0.07, 630);
    this.tone(1260, 0.035, 'triangle', 0.035, null, 0.025);
  }

  // Game over: dark spectral fall
  gameOver() {
    this.tone(330, 0.18, 'sawtooth', 0.10, 210);
    this.tone(220, 0.22, 'sawtooth', 0.095, 120, 0.18);
    this.tone(110, 0.34, 'sine', 0.10, 55, 0.38);
    this.noise(0.24, 0.06, 0.12, 300, 'lowpass');
  }
}
...
class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    const startGame = () => {
      ...
      // Keep the title/menu music playing into character select.
      // It fades out only when the player selects a hero and Battle starts.
  ...
  create() {
    ...
    chars.forEach((charKey, i) => {
      ...
      card.on('pointerdown', () => {
        ...
        fadeOutMenuMusic(this, 500);
        this.cameras.main.flash(180, 255, 255, 255, false);
        this.time.delayedCall(520, () => this.scene.start('Battle'));
      });
    });
